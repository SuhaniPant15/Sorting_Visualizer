...
function quick_partition (start, end)
{
    ...
    div_update(divs[start],div_sizes[start],"#3490DE");//Color update
        ...
        for(var j =start + 1; j <= end ; j++ )
        {
            ...
            if (div_sizes[ j ] < piv)
            {
                ...
                div_update(divs[j],div_sizes[j],"#4B6289");//Height update

                div_update(divs[i],div_sizes[i],"#3A1078");//Height update
                div_update(divs[j],div_sizes[j],"#3A1078");//Height update

                i += 1;
            }
    }
    div_update(divs[start],div_sizes[start],"#4B6289");//Color update
    ...
    div_update(divs[start],div_sizes[start],"#4B6289");//Height update
